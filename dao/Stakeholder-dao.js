...
exports.getActiveCollectionOfficers = () => {
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT COUNT(*) AS activeOfficerCount FROM collectionofficer WHERE  companyId = '1' AND status = 'Approved' AND onlineStatus = '1';
            `;
    collectionofficer.query(sql, (err, results) => {
      if (err) {
        return reject(err); // Reject promise if an error occurs
...
exports.SendGeneratedPasswordDao = async (
  ...
  try {
    ...
    doc
      ...
      .moveDown()
      .text(
        "You have successfully created an account with Polygon Holdings (Pvt) Ltd. Our platform provides agricultural support, guidance, and digital tools to help you grow and succeed.",
        { align: "justify" }
      )
      .moveDown()
      ...
      .moveDown()
      .text("Best Regards,")
      .text("The Polygon Holdings Team")
