...
exports.sendCenterComplainReply = (complainId, reply, adminId) => {
  return new Promise((resolve, reject) => {
    // Input validation
    if (!complainId) {
      return reject(new Error("Complain ID is required"));
    ...
    const sql = `
      ...
      SET reply = ?, COOStatus = ?, CCMStatus = ? , CCHStatus = ? , AdminStatus = ?, adminReplyBy = ?, replyTime = NOW()
      WHERE id = ?
    `;
...
exports.GetAllManagerList = (companyId, centerId) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT id, empId, firstNameEnglish, lastNameEnglish FROM collectionofficer WHERE companyId = ? AND centerId = ? AND jobRole = 'Collection Centre Manager'";
